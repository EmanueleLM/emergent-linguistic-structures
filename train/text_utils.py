...
def get_wikipedia_sample(n=1000):
    save_dst = f'./../data/datasets/wikipedia/'
    filename = f'wikipedia_sample_{n}-articles.txt'
    os.makedirs(save_dst, exist_ok=True)  # create the folder in case it doesn't exists
    print(f"Downloading {n} articles from wikipedia...")
    with open(save_dst+filename, 'w') as file_: 
        for i in tqdm.tqdm(range(n)): 
            #print(f"{i}/10000")
            valid = False
            while valid is not True:
                try:
                    f = random_wikipedia_page()
                    valid = True
                except:
                    valid = False
            file_.write(f+'\n')
    print(f"Saving file at `{save_dst}` with name `{filename}`")
