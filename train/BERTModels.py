...
class SentimentalBERT(nn.Module):
    """
    ...
    """
    def __init__(self, bert, maxlen, mlm=False):      
        super(SentimentalBERT, self).__init__()
        self.bert = bert
    ...
    def get_reps(self, sent_id, mask):
        """
        Get the internal representations of the mlm.
        """
        x = self.bert(sent_id, attention_mask=mask)
        return x
...
class LegalBERT(nn.Module):
    ...
    def forward(self, sent_id, mask):
        batch_size = len(sent_id)  # this must be independent from the `expected` batch_size
        x = self.bert(sent_id, attention_mask=mask)[0][:,-1,:]  # semi-last hidden layer, [CLS] token
        #x = self.flatten(x)
        x = self.fc1(x)
        x = self.dropout(x)
        x = self.relu(x)
